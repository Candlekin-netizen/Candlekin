...
<main class="main"><div class="wrap"><div class="meta"><div class="card"><div class="k">Public lifecycle</div><div class="v lime">${current==='PUBLIC_MINT'?'MINT_LIVE':current}</div></div><div class="card"><div class="k">Mainnet chain</div><div class="v">${CHAIN_ID}</div></div><div class="card"><div class="k">Candlekin CA</div><div class="v">${CONTRACT}</div></div><div class="card"><div class="k">SeaDrop</div><div class="v">${SEADROP}</div></div><div class="card"><div class="k">Mainnet RPC</div><div class="v ${rpcConfigured?'lime':'bad'}">${rpcConfigured?'CONFIGURED':'MISSING'}</div></div><div class="card"><div class="k">Market Genome</div><div class="v warn">PRODUCTION MAP PENDING</div></div><div class="card"><div class="k">Preview mode</div><div class="v warn">ADMIN ONLY</div></div></div>
<div class="toolbar">${buttons}</div><div class="note">Previewing a lifecycle here does <strong>not</strong> change the public lifecycle. Anonymous visitors continue to receive only the server-controlled public state. The public page never receives the internal selector. Market Lab demo logic is available here for visual review only; public production access stays blocked until the genome mapping is frozen.</div>
