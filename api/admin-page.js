...
</style></head><body><header class="top"><div class="wrap bar"><div class="brand">CANDLEKIN <span class="lime">// ADMIN</span></div><div class="spacer"></div><a class="btn" href="/admin">Curation</a><button class="btn" onclick="location.reload()">Refresh</button></div></header><main class="main"><div class="wrap"><div class="meta"><div class="card"><div class="k">Public lifecycle</div><div class="v lime">${current==='PUBLIC_MINT'?'MINT_LIVE':current}</div></div><div class="card"><div class="k">Mainnet chain</div><div class="v">${CHAIN_ID}</div></div><div class="card"><div class="k">Candlekin CA</div><div class="v">${CONTRACT}</div></div><div class="card"><div class="k">SeaDrop</div><div class="v">${SEADROP}</div></div><div class="card"><div class="k">Mainnet RPC</div><div class="v ${rpcConfigured?'lime':'bad'}">${rpcConfigured?'CONFIGURED':'MISSING'}</div></div><div class="card"><div class="k">Market Genome</div><div class="v warn">PRODUCTION MAP PENDING</div></div><div class="card"><div class="k">Preview mode</div><div class="v warn">ADMIN ONLY</div></div></div><div class="toolbar">${buttons}</div><div class="note">Previewing a lifecycle here does <strong>not</strong> change the public lifecycle. Anonymous visitors receive only the server-controlled public state. The public page never receives the internal selector. Market Lab demo logic remains admin-preview-only until the production mapping is frozen.</div><div class="preview"><div class="previewhead"><span>ADMIN PREVIEW</span><span id="viewLabel">Viewing: WHITELIST_OPEN</span></div><iframe id="frame" title="Candlekin lifecycle preview" src="/?adminPreview=1&phase=WHITELIST_OPEN"></iframe></div></div></main><script>const frame=document.getElementById('frame'),label=document.getElementById('viewLabel');document.querySelectorAll('[data-phase]').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('[data-phase]').forEach(x=>x.classList.remove('active'));button.classList.add('active');const phase=button.dataset.phase;label.textContent='Viewing: '+(phase==='PUBLIC_MINT'?'MINT_LIVE':phase);frame.src='/?adminPreview=1&phase='+encodeURIComponent(phase)}));</script></body></html>`;
}
