(function(){
  ...
  function walletButton(item,index){
    ...
    return `<button class="ck-wallet-option" type="button" data-ck-wallet="${index}"><span class="ck-wallet-icon">${icon}</span><span class="ck-wallet-copy"><strong>${esc(info.name||'Browser Wallet')}</strong><span>${esc(info.rdns||'Browser EVM wallet')}</span></span><span class="ck-wallet-arrow">→</span></button>`;
  }
  ...
  function open(){
    ...
    overlay.innerHTML=`<div class="ck-wallet-dialog" role="dialog" aria-modal="true" aria-label="Connect wallet"><div class="ck-wallet-head"><div><div class="eyebrow">Candlekin // wallet</div><h2>Connect wallet.</h2></div><button class="ck-wallet-close" type="button" aria-label="Close">×</button></div><div class="ck-wallet-body"><p class="ck-wallet-note">Choose a wallet detected in this browser, or use WalletConnect for QR and mobile wallet links. Candlekin never asks for your seed phrase or private key.</p><div class="ck-wallet-list">${items.length?items.map(walletButton).join(''):`<div class="ck-wallet-option ck-wallet-empty" style="cursor:default"><span class="ck-wallet-icon">!</span><span class="ck-wallet-copy"><strong>No browser wallet detected</strong><span>Use WalletConnect below, install a browser wallet, or open Candlekin inside a wallet browser.</span></span></div>`}</div><div class="ck-wallet-mobile"><button class="ck-wallet-option ck-wallet-reown" type="button" data-ck-reown><span class="ck-wallet-icon">QR</span><span class="ck-wallet-copy"><strong>More wallets / Mobile</strong><span>WalletConnect QR, mobile links and additional EVM wallets.</span></span><span class="ck-wallet-badge">${reownReady()?'READY':'LOADING'}</span></button></div><div class="ck-wallet-status" id="ckWalletStatus"></div><div class="ck-wallet-foot"><span>Robinhood Chain Mainnet</span><span>WalletConnect + browser wallets</span></div></div>`;
