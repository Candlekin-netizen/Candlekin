(function(){
  ...
  function providerItems(){
    ...
    return api.list().filter(item=>{
      ...
      const order=name=>/metamask/i.test(name)?1:/rabby/i.test(name)?2:/okx/i.test(name)?3:9;
  ...
  function open(){
    ...
    overlay.innerHTML=`<div class="ck-wallet-dialog" role="dialog" aria-modal="true" aria-label="Connect wallet"><div class="ck-wallet-head"><div><div class="eyebrow">Candlekin // wallet</div><h2>Connect wallet.</h2></div><button class="ck-wallet-close" type="button" aria-label="Close">×</button></div><div class="ck-wallet-body"><p class="ck-wallet-note">Choose a wallet detected in this browser. Candlekin never asks for your seed phrase or private key.</p><div class="ck-wallet-list">${items.length?items.map(walletButton).join(''):`<div class="ck-wallet-option" style="cursor:default"><span class="ck-wallet-icon">!</span><span class="ck-wallet-copy"><strong>No browser wallet detected</strong><span>Install MetaMask, Rabby, OKX, or open this site inside a wallet browser.</span></span></div>`}</div><div class="ck-wallet-mobile"><div class="ck-wallet-option"><span class="ck-wallet-icon">QR</span><span class="ck-wallet-copy"><strong>WalletConnect / Mobile</strong><span>Reown AppKit activation pending project ID.</span></span><span class="ck-wallet-badge">NEXT</span></div></div><div class="ck-wallet-status" id="ckWalletStatus"></div><div class="ck-wallet-foot"><span>Robinhood Chain Testnet // 46630</span><span>Desktop + mobile-ready shell</span></div></div>`;
